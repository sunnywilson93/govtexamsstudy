...
import { BILL_STEPS, type BillStep, type BillVote } from '@/data/polity/bill-steps'

// ──────────────────────────────────────────────────────────
// Bill SVG — paper document with red ribbon + optional stamp
// ──────────────────────────────────────────────────────────
function BillIcon({ size = 44, stamped = false }: { size?: number; stamped?: boolean }) {
  return (
    <svg width={size} height={size * 1.2} viewBox="0 0 40 48" style={{ display: 'block' }}>
      <defs>
        <linearGradient id="bill-paper-grad" x1="0" y1="0" x2="0" y2="1">
...
// ──────────────────────────────────────────────────────────
// VoteBar — counts up tallies on the active vote step
// ──────────────────────────────────────────────────────────
function VoteBar({ vote, active }: { vote: BillVote | undefined; active: boolean }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div
        className="mono"
        style={{ display: 'flex', gap: 14, marginTop: 6, fontSize: 11, color: 'var(--ink-2)' }}
      >
        <span>
          <span style={{ color: '#16a34a' }}>▮</span> Aye {f}
        </span>
        <span>
          <span style={{ color: '#dc2626' }}>▮</span> Nay {a}
        </span>
...
// ──────────────────────────────────────────────────────────
// Chamber — one of the three rooms the Bill passes through
// ──────────────────────────────────────────────────────────
function Chamber({
  kind,
  label,
  ...
  return (
    <div
      style={{
        ...
        background: active
          ? `color-mix(in oklch, ${accent} 7%, var(--paper))`
          : 'var(--paper)',
    ...
    >
      ...
      <div style={{ marginTop: 10, minHeight: 22 }}>{children}</div>
    </div>
  )
}

// ──────────────────────────────────────────────────────────
// BillVisualizer — main composition
// ──────────────────────────────────────────────────────────
interface BillVisualizerProps {
  stepIdx?: number
  playing?: boolean
}

export function BillVisualizer({ stepIdx = 0 }: BillVisualizerProps) {
  const step: BillStep = BILL_STEPS[stepIdx] ?? BILL_STEPS[0]
  ...
  const laneOrder: Record<typeof lane, number> = { pre: 0, lok: 1, rajya: 2, pres: 3 }
  const lanesAt: Record<typeof lane, number> = { pre: 4, lok: 70, rajya: 200, pres: 330 }
  ...
  return (
    ...
    >
      ...
      >
        {/* Left rail — travelling bill + waypoint dots */}
        <div style={{ position: 'relative', minHeight: 360 }}>
          <div
            style={{
              ...
              bottom: 18,
              width: 0,
              borderLeft: '1.5px dashed var(--rule-2)',
          ...
          <div
            ...
            style={{
              ...
              transition:
                'top var(--dur-slow) var(--ease-in-out-snap), transform var(--dur-base) var(--ease-out-soft)',
          ...
          {(['pre', 'lok', 'rajya', 'pres'] as const).map((l, i) => {
            const done = laneOrder[lane] >= i
            return (
              <div
                key={l}
                style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Chamber
            kind="lok"
            label="Lok Sabha"
            sub="House of the People"
            seats="543 seats"
            accent="var(--subj-polity)"
            active={lane === 'lok'}
            completed={lokDone}
          >
            {lane === 'lok' && (
              <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                {step.id === 'intro' &&
                  'Speaker reads the title of the Bill. No debate. Voice vote on motion to introduce.'}
                {step.id === 'lok-debate' &&
                  'Members debate clauses. Amendments tabled. Possibly referred to Standing Committee.'}
          ...
          >
            {lane === 'rajya' && (
              <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                {step.id === 'rajya-arrive' &&
                  'Three readings, mirroring Lok Sabha. Both Houses must pass identical text.'}
          ...
          <Chamber
            kind="president"
            label="Rashtrapati Bhavan"
            sub="President of India"
            seats="Art. 111"
            accent="var(--subj-polity)"
            active={lane === 'pres'}
            completed={presDone}
...
// ──────────────────────────────────────────────────────────
// HeroAutoVisualizer — cycles steps every 2.4s for the hero
// ──────────────────────────────────────────────────────────
export function HeroAutoVisualizer() {
  const [idx, setIdx] = useState(2)
  useEffect(() => {
    const t = setInterval(() => setIdx((i) => (i + 1) % BILL_STEPS.length), 2400)
    return () => clearInterval(t)
  }, [])
  return <BillVisualizer stepIdx={idx} playing />
}
