  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="rounded-full bg-subject-history-light px-3 py-0.5 text-xs font-medium text-subject-history-dark">
      ...
      >
        ...
        <div className="mt-4 flex flex-col gap-2">
          {problem.options.map((option, i) => {
            ...
            if (selectedOption !== null) {
              ...
              optionClass +=
                ' border-border-primary hover:border-subject-history hover:bg-subject-history-light text-text-primary'
            }
        ...
        {selectedOption !== null && (
          <motion.div
            ...
            className="mt-4 rounded-md border-l-4 border-subject-history bg-subject-history-light p-3"
          >
            <p className="text-sm text-subject-history-dark">{problem.explanation}</p>
          </motion.div>
        )}
      </motion.div>
      ...
      {selectedOption !== null && currentIndex < filtered.length - 1 && (
        <button
          onClick={handleNext}
          className="self-end rounded-md bg-subject-history px-4 py-2 text-sm font-medium text-white hover:opacity-90 transition-opacity cursor-pointer"
        >
          Next Question
        </button>
      )}
